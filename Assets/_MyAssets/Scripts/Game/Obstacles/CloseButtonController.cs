...
{
    CloseDoorController closeDoorController;
    [SerializeField] Transform button_transform;
    ...
    void Awake()
    {
        closeDoorController = transform.parent.GetComponent<CloseDoorController>();
    }
    ...
    void OnValidate()
    {
        button_transform.localPosition = new Vector3(buttonPosition.x, 0, buttonPosition.y);

        float posX = 3.9f;
        if (buttonPosition.x < 0) posX *= -1;
        Vector3 pos = button_transform.localPosition;
        pos.x = posX;
        pos.y = 0.25f;
        ...
        float length_h = (button_transform.localPosition.x - posX) / 2;
